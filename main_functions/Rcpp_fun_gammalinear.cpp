...
arma::vec dloss(arma::vec y, arma::mat x, double gamma, arma::vec beta, double sigma, arma::vec xbj, arma::mat xj){
   ...
   arma::mat xjt = xj.t();
   double wi=gamma*std::pow( (1+gamma)/2/pi/std::pow(sigma,2), gamma/2/(gamma+1) )/std::pow(sigma,2);
   for(int i=0; i<n; i++){
...
double objloss(arma::vec y, arma::mat x, double gamma, arma::vec beta, double sigma, arma::vec xbj){
   ...
   arma::vec ei(n);
   double pi = 3.1415926;
   double wi=std::pow( (1+gamma)/2/pi/std::pow(sigma,2), gamma/2/(gamma+1) );
   double objlossv = 0;
   for(int i=0; i<n; i++){
	   ei(i) = exp( -gamma*std::pow(y(i)-eta(i),2)/2/std::pow(sigma,2) );
	   objlossv = objlossv - wi*ei(i)/n;
   }
   return objlossv;
}
...
double biselect(arma::vec y, arma::mat x, double gamma, arma::vec beta, double min1, double max1){
   ...
   while(it < maxit && delta > toler){
       ...
       for(int i=0; i<n; i++){
           ...
           objlossa = objlossa + (std::pow(y(i)-eta(i),2)-std::pow(a,2)/(1+gamma))*ei(i);
       }
       objlossb = 0;
       for(int i=0; i<n; i++){
           ei(i) = exp( -gamma*std::pow(y(i)-eta(i),2)/2/std::pow(b,2) );
           objlossb = objlossb + (std::pow(y(i)-eta(i),2)-std::pow(b,2)/(1+gamma))*ei(i);
       }
       ...
       for(int i=0; i<n; i++){
           ...
           objlossab = objlossab + (std::pow(y(i)-eta(i),2)-std::pow(ab,2)/(1+gamma))*ei(i);
       }
...
List gammalinearsgMCP (arma::vec y, arma::mat x, arma::mat w, double gamma, arma::vec bhat0,
         double MCPlamgroup, double MCPlamsingle, double MCPa, int max_iter, double eps, int thre){
	int n = y.size();
    int q = x.n_cols;
    int p = w.n_cols/(q+1);
	int iter = 0;
