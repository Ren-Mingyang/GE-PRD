...
arma::vec dloss(arma::vec y, arma::mat x, double gamma, arma::vec beta, arma::vec xbj, arma::mat xj){
   int n = y.size();
   int l = xj.n_cols;
   arma::vec ei(n), wi(n), pi(n), dlossv(l), b_alpha(n);
   arma::vec eta = x*beta + xbj;
   arma::mat xjt = xj.t();
   for(int i=0; i<n; i++){
	   ei(i) = exp(eta(i));
	   b_alpha(i) = ei(i)*(std::pow(ei(i),gamma) - 1) / std::pow(1 + ei(i),2 + gamma);
	   wi(i) = std::pow( std::pow(ei(i),y(i))/(1+ei(i)) , gamma );
	   pi(i) = (1+gamma)*( b_alpha(i) - wi(i)*( y(i) - ei(i)/(1+ei(i)) ) )/n;
   }
   dlossv = xjt*pi;
   return dlossv;
}
...
double objloss(arma::vec y, arma::mat x, double gamma, arma::vec beta, arma::vec xbj){
   ...
   for(int i=0; i<n; i++){
	   ei(i) = exp(eta(i));
	   wi(i) = std::pow( std::pow(ei(i),y(i))/(1+ei(i)),gamma );
	   pi(i) = ( 1 + std::pow(ei(i),(1 + gamma)) ) / std::pow(1+ei(i),1 + gamma) - (1 + 1/gamma) * wi(i);
	   objlossv = objlossv + pi(i)/n;
   }
   return objlossv;
}
...
List mdpdlogisticsgMCP (arma::vec y, arma::mat x, arma::mat w, double gamma, arma::vec bhat0,
         double MCPlamgroup, double MCPlamsingle, double MCPa, int max_iter, double eps, int sear, double d_main){
	int n = y.size();
    int q = x.n_cols;
    int p = w.n_cols/(q+1);
	int iter = 0;
